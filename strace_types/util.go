...
// The modern prog API keeps the argument's type reference and direction inside
// unexported ArgCommon fields, so every arg must be built through the prog.Make*
// constructors, each of which takes an explicit direction.
func GroupArg(t prog.Type, dir prog.Dir, inner []prog.Arg) prog.Arg {
	return prog.MakeGroupArg(t, dir, inner)
}

func PointerArg(t prog.Type, dir prog.Dir, address uint64, vmaSize uint64, res prog.Arg) prog.Arg {
	if res == nil {
		return prog.MakeVmaPointerArg(t, dir, address, vmaSize)
	}
	return prog.MakePointerArg(t, dir, address, res)
}

func ConstArg(t prog.Type, dir prog.Dir, v uint64) prog.Arg {
	return prog.MakeConstArg(t, dir, v)
}

func DataArg(t prog.Type, dir prog.Dir, data []byte) prog.Arg {
	return prog.MakeDataArg(t, dir, data)
}

func UnionArg(t prog.Type, dir prog.Dir, opt prog.Arg, index int) prog.Arg {
	return prog.MakeUnionArg(t, dir, opt, index)
}

func ResultArg(t prog.Type, dir prog.Dir, r *prog.ResultArg, v uint64) prog.Arg {
	return prog.MakeResultArg(t, dir, r, v)
}
...
// DefaultValue replaces Type.Default(), which no longer exists on the modern
// Type interface. Only resource types carried a non-zero default.
func DefaultValue(t prog.Type) uint64 {
	if rt, ok := t.(*prog.ResourceType); ok {
		return rt.Default()
	}
	return 0
}
